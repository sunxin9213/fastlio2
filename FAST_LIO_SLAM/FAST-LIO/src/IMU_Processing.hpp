...
class ImuProcess
{
 ...
 private:
  ...
  PointCloudXYZI::Ptr cur_pcl_un_;//当前帧未去畸变点云
  ...
  bool   imu_need_init_ = true;//是否初始化imu
};
...
{
  init_iter_num = 1;
  Q = process_noise_cov();//ba， bg， biaa, biag的协方差
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  if (b_first_frame_)//第一帧，取队列中的最前数据
  {
    Reset();
    N = 1;
  ...
  {//sx:方差迭代计算公式
    ...
    mean_acc      += (cur_acc - mean_acc) / N;//平均加速度
    mean_gyr      += (cur_gyr - mean_gyr) / N;

    cov_acc = cov_acc * (N - 1.0) / N + (cur_acc - mean_acc).cwiseProduct(cur_acc - mean_acc) * (N - 1.0) / (N * N);
    cov_gyr = cov_gyr * (N - 1.0) / N + (cur_gyr - mean_gyr).cwiseProduct(cur_gyr - mean_gyr) * (N - 1.0) / (N * N);
    //cwiseProduct函数逐次计算元素乘积并加和
    // cout<<"acc norm: "<<cur_acc.norm()<<" "<<mean_acc.norm()<<endl;

    N ++;
  }
  state_ikfom init_state = kf_state.get_x();
  init_state.grav = S2(- mean_acc / mean_acc.norm() * G_m_s2);//s2是一个类中的对象
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--)
  {
    ...
    for(; it_pcl->curvature / double(1000) > head->offset_time; it_pcl --)
    {
      ...
      M3D R_i(R_imu * Exp(angvel_avr, dt));//当前激光时间的位姿，根据imu和dt计算出来的
      
      V3D P_i(it_pcl->x, it_pcl->y, it_pcl->z);
      V3D T_ei(pos_imu + vel_imu * dt + 0.5 * acc_imu * dt * dt - imu_state.pos);//imu_state.pos表示什么？predict的值
      V3D P_compensate = imu_state.offset_R_L_I.conjugate() * (imu_state.rot.conjugate() * (R_i * (imu_state.offset_R_L_I * P_i + imu_state.offset_T_L_I) + T_ei) - imu_state.offset_T_L_I);// not accurate!应该是论文的公式
