  ...
  if (feature_enabled)
  {
    ...
    for(uint i=1; i<plsize; i++)
    {
      ...
      if((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
      {
        ...
        {
          pl_buff[msg->points[i].line].push_back(pl_full[i]);//sx:pl_buff与pl_full的差别在于前者进行了距离判断，前者不保留重复点
        }
      }
    }
    static int count = 0;
    ...
    for(int j=0; j<N_SCANS; j++)
    {
      ...
      plsize = pl.size();
      vector<orgtype> &types = typess[j];//sx:对于每个线
      types.clear();
      types.resize(plsize);
      plsize--;
...
void Preprocess::velodyne_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    ...
    double omega_l=3.61;       // scan angular velocity，3.61度/ms，这里是10hz？？？
    ...
    else
    {
      ...
      double yaw_first = atan2(pl_orig.points[0].y, pl_orig.points[0].x) * 57.29578;//转换成角度
      ...
      for (uint i = plsize - 1; i > 0; i--)
      {
        ...
        {
          yaw_end = atan2(pl_orig.points[i].y, pl_orig.points[i].x) * 57.29578;//这一步骤只是确定了首尾点的角度
          break;
        }
      }
    }
    ...
    if(feature_enabled)
    {
      ...
      for (int i = 0; i < plsize; i++)
      {
        ...
        if (!given_offset_time)//如果点云是不带时间的
        {
          double yaw_angle = atan2(added_pt.y, added_pt.x) * 57.2957;//第一个点的角度
          if (is_first[layer])//取第一个点为时间原点
          {
          ...
          if (yaw_angle <= yaw_fp[layer])//顺时针计算时间
          {
            added_pt.curvature = (yaw_fp[layer]-yaw_angle) / omega_l;//距离上一个点的时间？
          }
          else
          {
      ...
      for (int j = 0; j < N_SCANS; j++)
      {
        ...
        vector<orgtype> &types = typess[j];//每一线数据
        types.clear();
        types.resize(linesize);
        linesize--;
        for (uint i = 0; i < linesize; i++)
        {
          types[i].range = sqrt(pl[i].x * pl[i].x + pl[i].y * pl[i].y);//水平投影长度
          vx = pl[i].x - pl[i + 1].x;
          vy = pl[i].y - pl[i + 1].y;
          vz = pl[i].z - pl[i + 1].z;
          types[i].dista = vx * vx + vy * vy + vz * vz;//前后两点空间距离
        }
        types[linesize].range = sqrt(pl[linesize].x * pl[linesize].x + pl[linesize].y * pl[linesize].y);
        give_feature(pl, types);
      }
...
{//对每线调用这个函数
  ...
  // 过滤盲区的点，sx:不应该是过滤，而是找到第一个不在盲区的点
  while(types[head].range < blind)//sx:在x-y平面上的投影距离，用while循环？？？
  {
    head++;
  }
  ...
  Eigen::Vector3d curr_direct(Eigen::Vector3d::Zero()); //当前平面的法向量，sx:这里就是向量，不是法向量
  ...
  // 拿到8个点用于判断平面
  for(uint i=head; i<plsize2; i++)//sx:从第一个不在盲区的点开始搜索
  {
    ...
    // 求得平面，并返回类型0 1 2， 2表示该点不是面点，1表示是面点
    plane_type = plane_judge(pl, types, i, i_nex, curr_direct);
    //sx:对每个点调用这个函数，0不是直线上的点，1是直线上的点，2表示该点附近有盲区，curr_direct表示直线的方向向量
    // 返回1一般默认是平面
    if(plane_type == 1)
    {
      //设置确定的平面点和可能的平面点
      for(uint j=i; j<=i_nex; j++)//sx:这里i_nex已经再plane_judge中改变了
      {
        // * * * * * *
        // i         i_nex
      ...
      if(last_state==1 && last_direct.norm()>0.1)//sx:这个norm的意义在哪？
      {
        // 计算cos(last_n, curr_n)
  ...
  for(uint i=head+3; i<plsize2; i++)
  {
    ...
    for(int j=0; j<2; j++)
    {
      ...
      types[i].angle[j] = vec_a.dot(vecs[j]) / vec_a.norm() / vecs[j].norm(); //角MAN的cos值，sx:这里应该是MAO的角度值吧
      ...
      {
        types[i].edj[j] = Nr_zero; // M在OA上，sx:这两个意思应该是说反了，但是整体理解不影响，就是M在OA或者其延长线上，判断是不是前后遮挡场景
      }
    }
  ...
  for(uint j=head; j<plsize; j++)
  {
    ...
    if(types[j].ftype==Poss_Plane || types[j].ftype==Real_Plane)
    {
      ...
      if(j == uint(last_surface+point_filter_num-1))
      {
        ...
        pl_surf.push_back(ap);//sx：这里对面点进行了降采样
...
{
  double group_dis = disA*types[i_cur].range + disB;//sx:0.01*range + 0.1，其实是一个阈值
  ...
  for(i_nex=i_cur; i_nex<i_cur+group_size; i_nex++)
  {
    ...
    disarr.push_back(types[i_nex].dista);//sx:先存8个点
  }
  
  for(;;)
  {//sx:如果循环体中没有结束条件，那么循环将不会停止
    ...
    two_dis = vx*vx + vy*vy + vz*vz;//sx:首尾两个点的距离
    if(two_dis >= group_dis)
    {
      break;//距离超过阈值了，结束此循环
    }
    disarr.push_back(types[i_nex].dista);//sx:8个点以后的点，只要首、尾点距离小于阈值，继续存，最多存20个？有没有可能存多余20个。
    i_nex++;
  }

  double leng_wid = 0;
  double v1[3], v2[3];
  for(uint j=i_cur+1; j<i_nex; j++)//sx:对存储再disarr内的点
  {
    ...
    v2[0] = v1[1]*vz - vy*v1[2];//vx、vy、vz是disarr中距离最长的两个点
    v2[1] = v1[2]*vx - v1[0]*vz;
    v2[2] = v1[0]*vy - vx*v1[1];//这里是进行了向量叉乘的计算

    double lw = v2[0]*v2[0] + v2[1]*v2[1] + v2[2]*v2[2];//叉乘的模为：a * b * sin(theta)，越大表示越不在直线上
    if(lw > leng_wid)
    {
      leng_wid = lw;//找到最大值
    }
  ...
  {
    curr_direct.setZero();//sx:leng_wid越大，表示不是一个平面
    return 0;
  }
  ...
  for(uint j=0; j<disarrsize-1; j++)
  {
    for(uint k=j+1; k<disarrsize; k++)
    {
      if(disarr[j] < disarr[k])
      {
        leng_wid = disarr[j];
        disarr[j] = disarr[k];
        disarr[k] = leng_wid;//应该是一个排序，小值在后
      }
    }
  }
